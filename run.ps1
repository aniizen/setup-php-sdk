...
function Get-SDKUrlStatusCode([string] $Url) { try{ (Invoke-WebRequest -Uri $Url -UseBasicParsing -DisableKeepAlive -Method head).StatusCode }catch [Net.WebException]{[int]$_.Exception.Response.StatusCode}}
...
$vs = $versions[$version.Split('.')[0,1] -join '.']
if (-not $vs) {
...
$phpversion = $releases[$version.Split('.')[0,1] -join '.']
if (3 -eq $version.Split('.').count) {
    $baseurl = "https://windows.php.net/downloads/releases/archives/"
    $phpversion = $version
    $fname = "php-$phpversion-$tspart-$vs-$arch.zip"

    $statusCode = Get-SDKUrlStatusCode "$baseurl/$fname"
    # $statusCode = try {
    #     (Invoke-WebRequest -Uri "$baseurl/$fname" -UseBasicParsing -DisableKeepAlive -Method head).StatusCode
    # }
